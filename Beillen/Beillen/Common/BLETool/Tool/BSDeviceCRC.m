...
uint16_t CrcValueByteCalc(const uint8_t *data, uint8_t length) {
...
@implementation BSDeviceCRC

+ (uint16_t)crcWithData:(NSData *)data
{
    if (!data || ![data isKindOfClass:[NSData class]]) return 0;
    UInt8 *crcData = (UInt8 *)[data bytes];
    return CrcValueByteCalc(crcData, data.length);
}
...
+ (NSString*)sumOfData:(NSData *)data {
    ...
    NSString *str = [NSString stringWithFormat:@"%02lx", sum & 0xFF];
